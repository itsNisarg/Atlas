...
class ATLASGame:
    ...
    def __init__(self, places, mode):
        ...
        self.hits_adv = self._safe_load(
            f"code/analysis/hits/{self.mode.lower()}_hits.csv", "Adv"
        )
        self.pagerank = self._safe_load(
            f"code/analysis/pagerank/{self.mode.lower()}_pg_rank.csv",
            f"{self.mode}PageRank",
        )
        self.trophic = self._safe_load(
            f"code/analysis/trophiclvl/{self.mode.lower()}_trophic.csv", "0"
        )
        self.degree_adv = self._safe_load(
            f"code/analysis/degree/{self.mode.lower()}_degree_adv.csv", "0"
        )
        self.betweenness_adv = self._safe_load(
            f"code/analysis/between/{self.mode.lower()}_betweenness.csv",
            f"{self.mode}Between",
        )
        self.avg_neighbour = self._safe_load(
            f"code/analysis/avg_neighbour/{self.mode.lower()}_avg_neighbours.csv",
            f"{self.mode}AvgNeighbour",
        )
    ...
    def get_ai_move(self, strategy="random"):
        ...
        if strategy == "pagerank" and self.pagerank is not None:
            return max(moves, key=lambda m: self.pagerank.get(m, 0))

        if strategy == "trophic" and self.trophic is not None:
            return min(moves, key=lambda m: self.trophic.get(m, 0))

        if strategy == "degree" and self.degree_adv is not None:
            return max(moves, key=lambda m: self.degree_adv.get(m, 0))

        if strategy == "betweenness" and self.betweenness_adv is not None:
            return max(moves, key=lambda m: self.betweenness_adv.get(m, 0))

        if strategy == "avg_neighbour" and self.avg_neighbour is not None:
            return min(moves, key=lambda m: self.avg_neighbour.get(m, 0))
...
def main():
    ...
    while True:
        ...
        # -------------------------------------------------
            ...
            col_means = matrix.mean(axis=0)  # Average of each column
            row_means = matrix.mean(axis=1)  # Average of each row
            ...
            # --- Top Axis (Column Averages) ---
            # Create a twin axis that shares the same x-scale
            ax_top = ax.secondary_xaxis("top")

            # Set the ticks to match the heatmap's column centers
            ax_top.set_xticks(ax.get_xticks())

            # Set the labels to be the calculated averages
            ax_top.set_xticklabels(
                [f"{m:.2f}" for m in col_means],
                rotation=45,
                ha="left",
                fontsize=10,
                color="blue",
            )
            ax_top.tick_params(axis="x", which="both", length=0)  # Hide tick lines

            # --- Right Axis (Row Averages) ---
            # Create a twin axis that shares the same y-scale
            ax_right = ax.secondary_yaxis("right")

            # Set the ticks to match the heatmap's row centers
            ax_right.set_yticks(ax.get_yticks())

            # Set the labels
            ax_right.set_yticklabels(
                [f"{m:.2f}" for m in row_means],
                rotation=0,
                fontsize=10,
                color="blue",
                va="center",
            )
            ax_right.tick_params(axis="y", which="both", length=0)  # Hide tick lines
